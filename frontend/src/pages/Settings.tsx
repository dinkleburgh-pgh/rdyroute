...
// GeneralPanel, PacePanel, ColorsPanel, WorkflowsPanel moved to components/management/
// ---------------------------------------------------------------------------
...
// CommunicationsPanel moved to components/management/CommunicationsPanel.tsx
// ---------------------------------------------------------------------------
// User Management
//
...
function ItemsPanel({ disabled }: { disabled: boolean }) {
  ...
  // Groups for display: each category + an "(none)" group for uncategorised
  const groups = useMemo(() => {
    ...
    // Sort: named categories alphabetically, then empty ("none") last
    const named = Array.from(map.entries())
      .filter(([k]) => k !== "")
      .sort(([a], [b]) => a.localeCompare(b));
    const uncategorised = map.get("") ?? [];
    return uncategorised.length > 0
      ? [...named, ["", uncategorised] as [string, TrackedItem[]]]
      : named;
  }, [draft]);
  ...
  function addItemToCategory(cat: string) {
    ...
    setDraft((d) => [
      ...d,
      { label, qty_default: Math.max(1, parseInt(addQty || "1", 10)), category: finalCat || undefined },
    ]);
    setAddLabel("");
    setAddQty("1");
    setAddCatNew("");
    setAddCatIsNew(false);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            onClick={() => { setDraft(items ?? []); setEditingLabel(null); }}
          >
            Revert
          </button>
        </div>
          ...
          <div key={cat || "__none__"} className="card space-y-2 p-3">
            ...
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs font-bold uppercase tracking-wide text-slate-400">
                {cat || "Uncategorised"}
                <span className="ml-1.5 font-normal text-slate-600">({catItems.length})</span>
              </span>
              {!disabled && (
                <button
              ...
              {catItems.map((it) => {
                ...
                if (isEditing) {
                  return (
                    <div
                      key={it.label}
                      className="flex w-full flex-wrap items-end gap-2 rounded-lg border border-blue-600/50 bg-slate-800/60 p-2.5"
                    >
                ...
                return (
                  <div
                    key={it.label}
                    className="group flex items-center gap-1 rounded-full border border-slate-700 bg-slate-800 pl-3 pr-1 py-1 text-sm text-slate-200 transition-colors hover:border-slate-600"
                  >
                    <button
                    ...
                    {it.qty_default !== 1 && (
                      <span className="ml-1 shrink-0 rounded-full bg-slate-700 px-1.5 py-0.5 text-[10px] font-semibold text-slate-400">
                        ×{it.qty_default}
                      </span>
                    )}
                    {!disabled && (
                      <button
                        ...
                        className="ml-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-slate-500 transition-colors hover:bg-red-500/20 hover:text-red-400"
                        title="Remove"
                      >
                        ×
                      </button>
            ...
            {isAdding && !disabled && (
              <div className="flex flex-wrap items-end gap-2 rounded-lg border border-slate-700 bg-slate-800/40 p-2.5">
                ...
                {/* Show category field only when viewing All or adding uncategorised */}
                  ...
                  addCatIsNew ? (
                    ...
                    <div className="w-36">
                      ...
                      <select
                        ...
                        value={cat}
                        onChange={(e) => {
                          if (e.target.value === "__new__") { setAddCatIsNew(true); }
                          else { setAddingToCategory(e.target.value); }
                        }}
                      >
                ...
                <button
                  className="btn-primary text-xs pb-1.5"
      ...
      {/* Global add (when no sections visible or adding to "All" view) */}
      {activeTab === "__all__" && (
        <div className="card p-3">
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            {addCatIsNew ? (
              ...
              <div className="w-40">
                ...
                <select
                  ...
                  disabled={disabled}
                  onChange={(e) => {
                    if (e.target.value === "__new__") { setAddCatIsNew(true); setAddingToCategory(null); }
                    else { setAddingToCategory(e.target.value); }
                  }}
                >
                  <option value="">— none —</option>
            ...
            <button
              className="btn-ghost"
              disabled={disabled || !addLabel.trim()}
              onClick={() => addItemToCategory(addCatIsNew ? addCatNew : addingToCategory ?? "")}
            >
              Add
            </button>
          </div>
      ...
      {importOpen && (
        <div className="card space-y-2">
          ...
          />
          <div className="flex gap-2">
            <button className="btn-primary" disabled={!importText.trim()} onClick={applyBulkImport}>
              Apply import
            </button>
