  ...
  return (
    <div className="space-y-4">
      <div className="card space-y-2">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-ink-soft">Driver QR codes</h3>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="inline-flex overflow-hidden rounded-md border border-hairline text-xs font-semibold print:hidden">
            {([["all", "All"], ["route", "Routes"], ["spare", "Spares"]] as const).map(([key, label], i) => (
              <button
                ...
                className={
                  ...
                  (typeFilter === key ? "bg-track text-white" : "bg-surface-2 text-ink-muted hover:bg-track")
                }
              >
                {label}
              </button>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 print:grid-cols-3">
        {active.map((truck) => {
          ...
          return (
            <div key={truck.truck_number}
              className="flex flex-col items-center gap-2 rounded-lg border border-hairline bg-surface p-3 print:border-slate-300 print:bg-white">
              <p className="text-sm font-semibold text-ink-soft print:text-slate-900">
              ...
              <div className="flex w-full gap-1 print:hidden">
                ...
                <button
                  className="shrink-0 rounded-md bg-track px-2 text-[11px] font-medium text-ink-soft hover:bg-track"
              ...
              <div className="flex w-full gap-1.5 print:hidden">
                <a href={url} target="_blank" rel="noopener noreferrer"
                  className="flex-1 rounded-md bg-surface-2 py-1 text-center text-[11px] font-medium text-ink-soft hover:bg-track">
                  Preview ↗
                </a>
                <button
              ...
              <p className="hidden text-center text-[10px] text-ink-faint print:block">
        ...
        {active.length === 0 && (
          <p className="col-span-full text-sm text-ink-muted">No active trucks match that filter.</p>
        )}
      </div>
      <ConfirmDialog
